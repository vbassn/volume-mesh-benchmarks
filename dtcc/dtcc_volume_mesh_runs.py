...
from build_volume_mesh_old import build_volume_mesh
...
run_results_df = (
    pd.read_csv(dtcc_runs_results_csv_path, index_col=0)
    if dtcc_runs_results_csv_path.exists()
...
for idx, combo in enumerate(itertools.product(*values), start=1):
    ...
    vertices, cells, num_buildings, elapsed = build_volume_mesh(
        parameters=params, save_meshes=True
    )
